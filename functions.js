...
function getAuthorByName(authorName, authors) {
  return authors.find(
    auth => auth.name.toUpperCase() === authorName.toUpperCase()
  );
}
...
function titlesByAuthorName(authorName, authors, books) {
  let titles = [];
  let auth_found = getAuthorByName(authorName, authors);
  let book_found;

  if (auth_found) {
    for (let i = 0; i < auth_found.books.length; i++) {
      book_found = getBookById(auth_found.books[i], books);
      if (auth_found.books[i] === book_found.id) {
        titles.push(book_found.title);
      }
    }
  }
  return titles;
